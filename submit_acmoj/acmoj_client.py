...
class ACMOJClient:
    ...
    def submit_file(self, problem_id: int, language: str, code: str) -> Optional[Dict]:
...
def main():
    ...
    # Direct file/code submission sub-command
    submit_file_parser = subparsers.add_parser("submit-file", help="Submit code from a local file")
    submit_file_parser.add_argument("--problem-id", type=int, required=True, help="Problem ID")
    submit_file_parser.add_argument("--language", type=str, required=True, help="Language, e.g., cpp, c, git")
    ...
    if args.command == "submit":
        ...
        try:
            ...
            return
        result = client.submit_file(args.problem_id, args.language, code)
